...
The Bash downloader must run without Python or PowerShell, must fail before its
first DNS request when a required command or embedded invariant is unavailable,
must remain silent unless `--verbose` is supplied, and must never write
unverified plaintext. Server-side generation must continue to work under
Python 2.7 and 3.x on Windows and Linux using only the Python standard library.
...
`--verbose`. Default to the embedded PSK, the system resolver used by `dig`, and
`${TMPDIR:-/tmp}/dnsdle_<file_id>`. Preserve binary stdout behavior for
`--out -`; otherwise stage plaintext beside the requested destination and use
`mv` only after all verification succeeds. Install a trap that removes all

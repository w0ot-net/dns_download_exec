...
by OS profile), but this invariant is not currently documented and enforced as a
single, explicit startup contract. Without a hard cardinality check and
file-bound artifact validation, review and future changes can drift into
ambiguous behavior.
...
- In `dnsdle/generator.py`, enforce deterministic artifact key uniqueness by
  `(file_id, target_os)` and deterministic filename uniqueness.
...
- In `dnsdle.py`, keep `generation_start` and `generation_summary` records tied
  to expected and realized artifact counts.
- Ensure per-artifact lifecycle log (`generation_ok`) carries
  `file_id`, `file_tag`, and `target_os` so operators can verify one-to-one
  coverage.
...
- Run startup with 2 files and `windows,linux`; verify exactly 4 artifacts and
  one artifact per `(file_id, target_os)`.

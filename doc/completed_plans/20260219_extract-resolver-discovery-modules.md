...
All plan items implemented as specified. Key implementation details:

- `_TEMPLATE_PREFIX` lost one trailing blank line so the leading `\n` from
  `_lift_resolver_source` output produces the correct 2-blank-line boundary.
- `_DISCOVER_SYSTEM_RESOLVER` uses a leading `\n\n` (not `\n\n\n`) to produce
  2 blank lines after the lifted loader source.
- Generated output verified byte-identical for both `linux` and `windows`
  targets before and after the change.
- Both resolver modules are independently importable (`from dnsdle.resolver_linux
  import _load_unix_resolvers`, etc.).

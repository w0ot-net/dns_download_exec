...
- Created `dnsdle/stager_generator.py` with `generate_stager()` and
  `generate_stagers()`. Pipeline: substitute -> verify no unreplaced
  placeholders -> minify -> compile-check -> zlib compress -> base64
  encode -> verify ASCII -> round-trip verify -> wrap in bootstrap
  one-liner.
- Updated `dnsdle/__init__.py`: partitions `combined_mapped` by matching
  `source_filename` against artifact filenames to extract client mapped
  items, calls `generate_stagers()`, returns 3-tuple
  `(runtime_state, generation_result, stagers)`.
- Updated `dnsdle.py`: unpacks 3-tuple, emits `stager_ready` log records
  per stager after generation logging.
- Updated `unit_tests/test_startup_state.py`: unpacks 3-tuple.
- Updated `unit_tests/test_startup_convergence.py`: added
  `generate_stagers` to `_PATCHABLE` with noop stub, added
  `source_filename` to all mapping stub return values, updated all
  unpack sites to 3-tuple.

Deviation from plan:
- `dnsdle/constants.py` was not modified. The RESOLVER/PSK placeholder
  tokens are literal strings embedded directly in the one-liner format
  string in `stager_generator.py` and are not shared across modules, so
  no constants were needed.

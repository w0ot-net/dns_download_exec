...
All seven phases implemented as specified:

1. **Phase 1**: Added `# __EXTRACT__` markers to `compat.py` (10 functions),
   `helpers.py` (2), `dnswire.py` (1), `cname_payload.py` (3).  Created
   `dnsdle/extract.py` with marker parser and identifier renamer.  Updated
   `constant_time_equals` to try `hmac.compare_digest` first.  Added
   `import hmac` to `compat.py`.

2. **Phase 2**: Created `dnsdle/client_standalone.py` with full universal
   client logic: PY2/type-detection preamble, inline constants, cross-platform
   resolver discovery (runtime `sys.platform` branch), CLI parser for all
   required/optional parameters, runtime derivation of `file_id`/`file_tag`/
   `slice_token`, download loop, reassembly, and `build_client_source()`
   assembler that extracts functions from canonical modules.  Assembled source
   is 1056 lines, compiles cleanly, and is ASCII-only.

3. **Phase 3**: Rewrote `client_generator.py` to call `build_client_source()`
   once and produce a single artifact (`dnsdle_universal_client.py`).  Removed
   all template substitution machinery (`_render_client_source`,
   `_build_artifacts`, `_filename_for`, `_sanitize_filename_label`,
   `_validate_publish_item`).  Signature changed from
   `generate_client_artifacts(runtime_state)` to
   `generate_client_artifacts(config)`.

4. **Phase 4**: Rewrote `stager_template.py` to embed both resolver
   implementations inline (no more `_lift_resolver_source` dependency on
   `client_template.py`), added payload metadata placeholders
   (`PAYLOAD_PUBLISH_VERSION`, `PAYLOAD_TOTAL_SLICES`,
   `PAYLOAD_COMPRESSED_SIZE`, `PAYLOAD_SHA256`, `PAYLOAD_TOKEN_LEN`,
   `FILE_TAG_LEN`, `DOMAINS_STR`), and updated `exec()` tail to construct
   `sys.argv` with all payload params.  `build_stager_template()` no longer
   takes `target_os`.  Updated `stager_generator.py` signature to accept
   `client_publish_item` + `payload_publish_item` instead of iterating over
   artifacts.  Updated `stager_minify.py` rename table with 7 new entries.

5. **Phase 5**: Rewrote `__init__.py` convergence loop: builds one universal
   client via `generate_client_artifacts(config)`, publishes it as a single
   file, partitions combined mapped items into `client_mapped_item` (single)
   and `payload_mapped_items` (user files), generates one stager per user
   file via updated `generate_stagers()`.  Removed `_GeneratorInput` class.

6. **Phase 6**: Rewrote `CLIENT_GENERATION.md` and `CLIENT_RUNTIME.md` for
   universal client architecture.  Updated `CONFIG.md` (removed `target_os`),
   `ARCHITECTURE.md` (updated generation line), `ERRORS_AND_INVARIANTS.md`
   (updated generator contract invariants), `LOGGING.md` (updated generation
   event fields).

7. **Phase 7**: Deleted `client_template.py`.  Removed `target_os`,
   `target_os_csv` from `Config` namedtuple and `_normalize_target_os` from
   `config.py`.  Removed `--target-os` from `cli.py`.  Removed
   `ALLOWED_TARGET_OS` and `GENERATED_CLIENT_FILENAME_TEMPLATE` from
   `constants.py`.

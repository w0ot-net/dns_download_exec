...
- Extracted `_build_single_publish_item()` private helper from
  `build_publish_items()`. The helper accepts `item_context` dict for
  structured error context.
- Rewrote `build_publish_items()` to use the helper. Added optional
  `seen_plaintext_sha256` and `seen_file_ids` parameters (default `None`).
  Existing callers pass no extra args and are unaffected.
- Added `build_publish_items_from_sources()` using the same helper.
  Includes its own diagnostic logging with `source_index` and
  `source_filename` context.
- Added `"source"` and `"filename"` fields to the dicts returned by
  `generate_client_artifacts()` in `client_generator.py`.
- All 131 existing tests pass with no modifications.
